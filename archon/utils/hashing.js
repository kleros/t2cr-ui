  ...
  if (fileResponse.status !== 200)
    throw new Error(
      errorConstants.HTTP_ERROR(
        `Unable to fetch file at ${fileURI}. Returned status code ${fileResponse.status}`
      )
    );
...
export const validMultihash = (
  ...
  const hashFunction = customHashFunction || hashFunctions[decodedHash.code];
  if (!hashFunction)
    throw new Error(
      errorConstants.VALIDATION_ERROR(
  ...
  let fileHash = hashFunction(file);
  ...
  if (decodedHashHex.indexOf("0x") !== 0)
    decodedHashHex = `0x${decodedHashHex}`;
...
export const multihashFile = (
  ...
  const hashFunction = customHashFunction || hashFunctions[multicode];
  if (!hashFunction)
    throw new Error(`Hashing Error: Unsupported multicode ${multicode}`);

  const fileHash = hashFunction(file);

...
function LayoutRow({ as, sx, left, middle, right }) {
  return (
    ...
    >
      {left && <LayoutColumn>{left}</LayoutColumn>}
      {middle && <LayoutColumn justifyContent="center">{middle}</LayoutColumn>}
      {right && <LayoutColumn justifyContent="flex-end">{right}</LayoutColumn>}
    </Flex>
  );
}
